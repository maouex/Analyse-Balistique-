...
export function TopScoresWidget({ size = 'M' }: { size?: WidgetSize }) {
  ...
  if (ranked.length === 0) {
    return <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 4, color: 'var(--muted)' }}><Trophy size={18} color="var(--border-light)" /><span style={{ fontSize: 9, fontFamily: 'var(--font-mono)' }}>Aucun</span></div>;
  }
      ...
      {ranked.map((m, i) => (
        <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 6px', background: i === 0 ? 'rgba(255,170,0,0.06)' : 'var(--surface2)', border: `1px solid ${i === 0 ? 'rgba(255,170,0,0.2)' : 'var(--border)'}` }}>
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div style={{ fontSize: 7, color: 'var(--muted)', fontFamily: 'var(--font-mono)' }}>{m.calibre} — {m.snap?.nbImpacts} imp.</div>
          </div>
          <div style={{ fontSize: 13, fontWeight: 800, flexShrink: 0, fontFamily: 'var(--font-mono)', color: (m.snap?.score ?? 0) >= 80 ? 'var(--accent2)' : (m.snap?.score ?? 0) >= 60 ? 'var(--green)' : 'var(--amber)' }}>{m.snap?.score}</div>
        </div>
      ))}
    </div>
  );
